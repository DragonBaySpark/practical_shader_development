...
void ofApp::setup()
{
	...
	ofDisableArbTex();
	ofEnableDepthTest();
	cubeMesh.load("cube_triangle.ply");
	cube28VertexMesh.load("cube_28_vertex.ply");
	cubeShader.load("cube.vert", "cube.frag");
...
{
	using namespace glm;
	cameraData.position=vec3(0, 0, 2);
	cameraData.fov=radians(90.f);
	float aspect=1024.0f/768.0f;

	//mat4 model=glm::rotate(1.0f,vec3(1,1,1))*scale(vec3(0.5,0.5,0.5));
	mat4 model = glm::translate(vec3(-0.7,0, 0)) * glm::rotate(glm::radians(30.0f), vec3(0,1, 0)) * scale(vec3(1.0, 1.0, 1.0));
	...
	mat4 model2 = glm::translate(vec3(0.7,0,0))*glm::rotate(glm::radians(30.0f), vec3(0,1, 0)) * scale(vec3(1.0, 1.0, 1.0));
	//mat4 model = glm::mat4(1.0f)*scale(vec3(0.5, 0.5, 0.5));
	mat4 view2=glm::inverse(glm::translate(cameraData.position));
	mat4 proj2=glm::perspective(cameraData.fov,aspect,0.01f,100.0f);
	mat4 mvp2=proj2*view2*model2;

	cubeShader.setUniformMatrix4f("mvp", mvp2);

	cube28VertexMesh.draw();
	cubeShader.end();
}
//--------------------------------------------------------------
void ofApp::update()
