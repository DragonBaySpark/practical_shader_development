...
void ofApp::setup()
{
	...
	cubemap.load(	"night_front.jpg", "night_back.jpg", 
		...
		"night_top.jpg", "night_bottom.jpg"); // 加载立方体贴图



	shieldMesh.load("shield.ply");
	calcTangents(shieldMesh); // 计算切线
	shieldShader.load("shield.vert", "shield.frag");
	...
	waterLightData.direction = glm::vec3(0.5f, -1.0f, -1.0f); // 光源方向
	waterLightData.color = glm::vec3(1.0f, 1.0f, 1.0f); // 白色
	waterLightData.intensity = 1.0f; // 强度
	
	waterMesh.load("plane.ply");
	calcTangents(waterMesh); // 计算切线
	waterShader.load("water.vert", "water.frag");
	waterNormalImage.load("water_nrm.png");
	waterNormalImage.getTexture().setTextureWrap(GL_REPEAT, GL_REPEAT);

	pointLightData.position = glm::vec3(0.0f, -0.0f, 0.30f); // 光源位置
	pointLightData.color = glm::vec3(1.0f, 1.0f, 1.0f); // 白色
	pointLightData.intensity =1.0f; // 强度
	pointLightData.radius = 0.50f; // 半径


	
	spotLight.color = glm::vec3(1, 1, 1);
	spotLight.position = glm::vec3(0,0.75,1.0) + glm::vec3(0.0, 0, 0);
	spotLight.intensity = 1.0;
	spotLight.direction =glm::vec3(0, 0, -1);
	spotLight.cutoff = glm::cos(glm::radians(45.0f));
}
//--------------------------------------------------------------
...
void ofApp::drawCubemap()
{
	...
	mat4 model = glm::rotate(glm::radians(0.0f), vec3(1,0, 0)) * scale(vec3(1, 1, 1));
...
void ofApp::drawShield()
{
	using namespace glm;
	cameraData.position=vec3(0, 0.75f, 1.0f);
	cameraData.fov=radians(90.f);
	float aspect=1024.0f/768.0f;
	...
	//mat4 model=glm::rotate(1.0f,vec3(1,1,1))*scale(vec3(0.5,0.5,0.5));
	mat4 model =glm::translate(mat4(1.0f),vec3(0.0f,0.75f,0.0f));//* glm::rotate(glm::radians(0.0f), vec3(1,0, 0)) * scale(vec3(1.0, 1.0, 1.0));
	//mat4 model = glm::mat4(1.0f)*scale(vec3(0.5, 0.5, 0.5));
	//mat4 view=glm::inverse(glm::translate(cameraData.position))*rotate(glm::radians(-10.0f*(float)(ofGetElapsedTimef())), vec3(1,0, 0));
	mat4 view = glm::inverse(glm::translate(cameraData.position));// *glm::rotate(glm::radians(0), vec3(1, 0, 0));
	mat4 proj=glm::perspective(cameraData.fov,aspect,0.01f,100.0f);
	mat4 mvp=proj*view*model;
	...
	shieldShader.setUniform3f("lightPos", spotLight.position); // 光源位置
	shieldShader.setUniform3f("lightCol", spotLight.color* spotLight.intensity);
	shieldShader.setUniform3f("lightConeDir", spotLight.direction);
	shieldShader.setUniform1f("lightCutoff", spotLight.cutoff);
...
void ofApp::drawWater()
{
	...
	//mat4 model=glm::rotate(1.0f,vec3(1,1,1))*scale(vec3(0.5,0.5,0.5));
	mat4 model = glm::translate(mat4(1.0f), vec3(0,0, 0.0f)) * glm::rotate(glm::radians(-30.0f), vec3(1,0, 0)) * scale(vec3(0.75, 0.75, 0.75));
	//mat4 model = glm::mat4(1.0f)*scale(vec3(0.5, 0.5, 0.5));
	//mat4 view=glm::inverse(glm::translate(cameraData.position))*rotate(glm::radians(-10.0f*(float)(ofGetElapsedTimef())), vec3(1,0, 0));
	mat4 view = glm::inverse(glm::translate(cameraData.position)) * rotate(glm::radians(-10.0f ), vec3(1, 0, 0));
	mat4 proj=glm::perspective(cameraData.fov,aspect,0.01f,100.0f);
	...
	vec3 lightDirection = getLightDirection(waterLightData); // 获取光源方向
	...
	waterShader.setUniform3f("lightDir", lightDirection);
	waterShader.setUniform3f("lightCol", waterLightData.color*waterLightData.intensity);

	waterShader.setUniform3f("meshCol", vec3(0.3,0.3,0.4));
	waterShader.setUniform3f("meshSpecCol", vec3(1,1,1));
	waterShader.setUniformMatrix4f("setUniformMatrix4f", model); // 模型矩阵
	waterShader.setUniform3f("rimCol", vec3(0,1,0)); //轮廓颜色
	waterShader.setUniform3f("ambientCol", vec3(0.5,0.5,0.5)); // 环境光颜色
	waterShader.setUniform3f("cameraPos",cameraData.position); //相机位置

	time+=ofGetLastFrameTime();
	waterShader.setUniform1f("time", time);
	waterShader.setUniformTexture("normalTex", waterNormalImage.getTexture(), 0); // 纹理

	waterShader.setUniformTexture("envTex", cubemap.getTexture(), 1); // 立方体贴图
...
{
	if (key == 'a') {
		spotLight.position.x -= 0.0001f;
	}
	else if (key == 'd') {
		spotLight.position.x += 0.0001f;
	}
	else if (key == 'w') {
		spotLight.position.y += 0.05f;
	}
	else if (key =='s') {
		spotLight.position.y -= 0.05f;
	}
	else if (key == 'q') {
		spotLight.position.z -= 0.05f;
	}
	else if (key == 'e') {
		spotLight.position.z += 0.05f;
	}
	else if (key == 'z') {
		spotLight.cutoff-=glm::radians(0.5f);	
	}
	else if (key == 'x') {
		spotLight.cutoff+=glm::radians(0.5f);	
	}

	printf("spotLight.position.x = %f , spotLight.position.y = %f, spotLight.position.z = %f ,spotLight.cutoff=%f \n", spotLight.position.x, spotLight.position.y, spotLight.position.z, spotLight.cutoff);
